...
#####1. O que é?
A técnica do design atômico é baseada na criação de componentes reaproveitáveis no desenvolvimento de um sistema (web, mobile, etc), a fim de centralizar cada funcionalidade num único local para permitir seu reuso e diminuir seu tempo de manutenção.</br></br>
Esta metodologia para a criação de interfaces foi criada pelo webdesigner Brad Frost. Ela consiste em tomar emprestados da química os conceitos de átomos, moléculas e organismos, que juntos formam a matéria final (que é o sistema final entregue ao cliente).</br></br>
